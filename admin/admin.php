<?php 
    ...
    $akun_admin = query("SELECT * FROM user WHERE status='non aktif'");
?>
...
<html>
	<head>
		...
		<title>Uniwara Class - Konfirmasi Akun Admin</title>
		<!-- Bootstrap -->
            ...
            <div class="main">
                ...
                <main class="content px-3 py-4">
                    <div class="container">
                        <div class="row my-2">
                            <div class="col-md">
                                <h3 class="fw-bold text-uppercase">Konfirmasi Akun Admin</h3>
                                <hr>
                            </div>
                        </div>
                        <div class="row my-3">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><?= $row['jabatan']; ?></td>
                                                <td> 
                                                    <a href="akun.php?aksi=aktif-admin&id_user=<?= $row['id_user']; ?>" class="btn btn-success btn-sm" style="font-weight: 600;" onclick="return confirm('Apakah anda yakin ingin mengaktifkan akun <?= $row['nama']; ?> ?');"><i class="bi bi-person-fill-check"></i>&nbsp;Terima</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                        ...
                        <?php
                        if(isset($_GET['aksi']) == 'aktif-admin'){
                            $id_user = $_GET['id_user']; 
                            $cek=$koneksi->query("SELECT * FROM user WHERE id_user='$id_user'"); 
                            if($cek->num_rows == 0){ 
                            $pesan = '';
                            }else{
                                $insert = $koneksi->query("update user set status='aktif' where id_user='$id_user'") or die(mysqli_error());
                                if($insert){
                                    echo '<script language="javascript">alert("Data berhasil diaktifkan!"); document.location="admin.php";</script>';
                                }else{ 
                                    echo '<script language="javascript">alert("Data gagal diaktifkan!"); document.location="admin.php";</script>';
                                }
                            }
                        }else{
                            $pesan = '';
